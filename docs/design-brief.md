...
│           └── ConvergenceHero.tsx     (decorative hero, derived from DIAGRAM 1)
...
| 12 | Homepage `SelectedWork` entry for the case study (fifth row, `revealIndex={4}`) | Pending |
...
- [x] **Prompt 11 (Case study visuals)** — All five slots filled. Four new files in `src/components/diagrams/` plus `DiagramPrimitives.tsx`; `CaseStudyLayout.tsx` and the content file modified. Built in four gated stages, with a stop for review after the before/after pair.
  ...
  - **Open decision — dark mode and the raster.** The screenshot's light surround becomes a bright slab against the charcoal page, noticeably heavier than anything else on the piece. `WorkRow.tsx` already solves this for the homepage with hand-authored `-dark` plates swapped via `<picture>`, but that pattern exists precisely because **`next/image` has no `prefers-color-scheme` art-direction API** — adopting it here would mean dropping `next/image` for this one figure, against the prompt's spec. Left as-is and flagged rather than changed unilaterally. Options if parity matters: supply a dark-UI capture plus `<picture>` (costs `next/image`), or accept that a screenshot of a light interface is light.
  - **Figure captions still provisional** — untouched this prompt, per Edwin, and to be finalised now the visuals are settled.
  - **Verification.** `npm run build` and `npm run lint` clean throughout. No hardcoded hex in `src/components/diagrams/` apart from the documented mask channels. No duplicate ids page-wide, so desktop and mobile marker defs are not colliding. Heading order h1 → seven h2s, no skips; 4/4 figures carry a real `figcaption`; 6/6 body diagram SVGs carry `role="img"` with `<title>` and `<desc>`; the hero carries none of those and sits inside `aria-hidden` with zero focusable descendants. Computed styles confirm **zero animation and zero transition inside every diagram**, so the existing `[data-reveal]` reduced-motion override in `globals.css` remains the only motion mechanism. No horizontal overflow at 375, 600, 768 or 1280, light and dark. **Lighthouse was not run** — unavailable in the build environment; the structural properties it would assess were checked instead, and the score is unconfirmed.
...
8. **OG cache bust.** Run LinkedIn Post Inspector on `https://www.edwincw.com` and `https://www.edwincw.com/about`. LinkedIn caches OG metadata aggressively (often 7+ days) — the inspector forces a refresh. Slack/iMessage caches expire on their own; no action needed.
