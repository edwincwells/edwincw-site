...
| 14 | Salli case study — second essay at `/work/salli-agentic-ai`, prose final, six visuals as placeholders | 🚧 Visuals outstanding — not deployable |
| 14.1 | Salli visual 6 — the initiation matrix (`InitiationMatrix.tsx`) | 🚧 One of six built; five still placeholders |
...
| 16 | Homepage R&R row removed + nav "Case Studies" → internal "Work" anchor | ✅ Complete |
| 17 | Salli vocabulary — Cowork → Focus, focuses → agents | ✅ Complete |
| 17.1 | Salli "Chat and Focus" — two prose edits reconciling Chat with the agent library | ✅ Complete |
...
- [ ] **Prompt 14 (Salli case study)** — 🚧 **Not complete, and not deployable.** The second case study lives at `/work/salli-agentic-ai` ("Capability is not the product — Designing agentic AI around the moment a frontline manager decides what to do next"). The prose is final; **all six visuals are empty placeholders** and no caption has been written. Do not merge to `main`, and do not read Salli as finished anywhere else in this document. Two files created (`SalliAgenticAiContent.tsx`, plus the registry entry), one modified.
  - **The route absorbed it with no changes at all.** Prompt 10 built `/work/[slug]` to carry more than one essay, and that held exactly: one registry entry in `caseStudies.ts` plus one content file, and `generateStaticParams` picked the slug up on its own. `page.tsx`, `CaseStudyLayout.tsx`, `PullQuote.tsx` and `Container.tsx` are untouched — **no new variant, no new `Container` width, no new block type.** The build emits `/work/[slug]` as SSG with two paths. The registry comment's forward-looking "Salli adds one entry" was rewritten to the general case now that it has happened.
  - **`variant="placeholder"` does not exist, and was not added.** The build prompt asked for `<Figure variant="placeholder">`, but `FigureProps` is a `svg | raster` union. Prompt 10's actual mechanism is to pass `variant` + `placeholder` + `aspect` with no child and no `image`, and `Figure` falls through to `<Placeholder>` — so each slot carries the variant of its **eventual** type (2, 4, 5 raster; 3, 6 svg) and filling it later needs no variant change. Flagged rather than built, per the standing instruction to stop and say so.
  - **Captions are unwritten, not provisional.** Prompt 10 authored placeholder caption prose and lived with it for a whole prompt. Edwin's call this time: captions are visible copy in the site voice and are not Claude's to write. `caption` is required and the `<figcaption>` always renders, so each slot carries a bracketed TODO in the same convention as its label — `caption="[ Caption TBC — focus library and switching ]"`. Both the labels and the captions have to be replaced when the visuals land; an empty string was rejected because it renders an empty `<figcaption>`.
  - **Six slots, and this time six were actually specified.** Hero as a `Placeholder` in `heroVisual` (there is no Salli equivalent of `ConvergenceHero`, and none was built), five `Figure`s at the 920px `Measured` breakout. Measured in the rendered DOM at 1280: hero 1184×507 (2.33 = 21/9), visuals 2, 3 and 6 at 824×464 (16/9), visuals 4 and 5 at 824×515 (16/10). At 375: hero 16/9, all five at 4/3, no horizontal overflow at either width.
  - **Visuals 5 and 6 are separated by prose, deliberately.** The build prompt put them back to back in "Chat and Cowork". Edwin split them: visual 5 sits above "The idea came from one of our PMs…", visual 6 above "It's in final design now…", so neither section runs two figures with no reading between them.
  - **Prose fidelity was verified mechanically, not by eye.** As built, the rendered HTML was stripped to text and diffed against the source: **all 33 blocks matched verbatim and in the same order**, zero missing, zero extra. The only rendered difference was the three pull quotes carrying the curly quotation marks `PullQuote` supplies. Greps confirm no straight apostrophes anywhere in the content file, and "one in six", "three quarters" and "a few per cent" each appear exactly once — the figures are final and the wrapping had to be reflowed once to stop Prettier splitting "a few per cent" across a line break, which is worth knowing before anyone reformats this file. **That diff describes the file as first built, not as it now stands** — see the copy corrections below.
  - **Three copy corrections after the diff, at Edwin's direction.** The sixth `<Heading>` changed from "Who starts" to **"The same failure twice"**; the paragraph under it changed from "Two failures, and eventually it was obvious they were the same failure." to **"The library and the alerts failed for the same reason, and it took us longer than it should have to see it."**; and one prose instance of "focus library" became **"agent library"**. Figure labels and captions keep the original wording, so `[ Caption TBC — focus library and switching ]` is deliberate and not a missed rename. Nothing structural moved: re-verified in the DOM as 1×h1 → 7×h2 with no skips, five figures, ten reveal targets, the "who starts, and when" pull quote and its echo in the closing paragraph both untouched. Anyone re-running the fidelity diff against the original build prompt will now see these three blocks differ, and that is expected.
  - **One straight apostrophe was left alone, in the metadata.** The `description` in `caseStudies.ts` reads "How Salli's agentic AI companion…" with a straight `'`, because the build prompt supplied the registry entry verbatim as a code block. Every other apostrophe on the site is curly. It is not rendered as page copy, only as `<meta>`, but it is inconsistent — flagged for Edwin rather than silently changed.
  - **Structure:** h1 → seven h2s, no skips. Ten scroll-reveal targets — header, hero, five figures, three pull quotes — and **zero** on prose paragraphs, matching Prompt 10's decision that a long article of popping paragraphs is the wrong register. No `"use client"` directive in the content file; the header comment mentions the string, exactly as `RepositioningProductDesignContent.tsx` does. No new CSS, keyframes or transitions.
  - **Verification, and its one gap.** Build and lint clean. `/work/salli-agentic-ai` 200, `/work/nope` still 404, `/work/repositioning-product-design` 200 and unchanged. Prose measures **584px inside a 680px column, identical to `/about`** measured in the same session. Console clean apart from the pre-existing dev-mode font-preload warnings; the only 404 in the network log is the deliberate `/work/nope` probe. Scroll reveal was confirmed firing **one element at a time in document order as each entered the viewport** — nine of the ten, driven through the real input path at 375px. The tenth (visual 6) could not be reached: the preview pane went `visibilityState === "hidden"` partway through and stopped accepting scroll, the same limitation recorded in Prompt 13.1. Programmatic `window.scrollTo` does **not** substitute — it leaves every observer unfired, on this page and on the leadership study alike, which is worth knowing before anyone reads a wall of `data-revealed="false"` as a bug.
- [ ] **Prompt 14.1 (Salli visual 6 — the initiation matrix)** — 🚧 **One of six visuals built; the other five are still placeholders and the page is still not deployable.** `src/components/diagrams/InitiationMatrix.tsx` created, visual 6's `Figure` swapped from a placeholder to the component, and this log updated. Three files.
  - **The composition is the argument.** Two independent dimensions — who initiates the session, who sets the content — not one spectrum. A slider or a single axis would assert that Cowork is a midpoint between the agent library and the alerts, which is exactly the reading the essay is arguing against, so it is a plain cross with four plotted corners instead. **No frame and no boxed grid:** a box reads as a bounded space with an inside and an outside, when the point is two open-ended dimensions.
  - **The empty corner does the work.** The four positions form a rectangle — library and Cowork share a y, alerts and the vacant combination share a y, library and the vacant combination share an x, Cowork and alerts share an x — so the missing combination reads as a hole in a grid rather than as something merely absent. It carries **no dot and no number**: a dot would plot it as a fourth mode, a number would put it in the sequence.
  - **Two token questions the build prompt left open.** It specified `var(--color-text)`, which does not exist — the tokens are `--color-foreground`, `--color-body`, `--color-muted` — so the non-accent point labels use **`--color-foreground`**, matching `NODE_LABEL_STYLE` in `DiagramPrimitives.tsx` so this figure sits with the other four rather than apart from them. "The site's existing accent token" is also ambiguous, since `globals.css` defines two: Edwin's call was **`--color-primary`** (teal), on the grounds that ochre appears exactly once site-wide, in `HeroDiagram`'s travelling signal, while teal is what every other diagram, eyebrow and link already means by "accent".
  - **Mobile is a real HTML `<table>`, not SVG text rows.** The build prompt asked which route `Figure` allowed: its `svg` variant is typed `children?: ReactNode` and renders them directly, so **arbitrary JSX is accepted** and the table was the better option — the type holds a true 14px instead of scaling with a viewBox, cells wrap properly, and a screen reader gets real row and column semantics instead of a flat list of `<text>` nodes. Precedent already existed in `DeliveryModelPair`'s `StateLabel`, which is deliberately HTML inside a diagram for the same reason. The vacant combination is **dropped from the table** rather than shown as an empty row: on desktop its position in the grid is the argument, but in a list of modes it would just be a blank line, and the axes it demonstrates are already named in the column headers. The table's `<caption>` names the table rather than reusing `DESC`, which narrates the cross's left and right and would be actively misleading read against rows.
  - **`viewBox="0 0 824 464"` is not arbitrary** — it is the slot's measured box at 1280px, so the drawing renders at **scale 1.000** and 14 units is a true 14px. Verified in the DOM: the `<svg>` measures 824×464 and all eight `<text>` elements compute to `14px`.
  - **No font family is set,** per the build prompt, unlike the other four diagrams, which pass a module-private `FONT_STACK` that `DiagramPrimitives.tsx` does not export. The `<text>` elements inherit from `body` — confirmed rather than assumed: computed `font-family` on a label resolves to `generalSans`.
  - **Reuses `DiagramSvg`** for the shell, which already supplies `role="img"`, `aria-label`, `<title>`, `<desc>`, `preserveAspectRatio` and responsive sizing, so the accessibility requirement needed nothing new. Colours all go through `style` rather than presentation attributes — `fill="var(…)"` does not resolve in any current browser, the constraint `DiagramPrimitives.tsx` documents at the top.
  - **Type size, stated rather than glossed.** 14px holds at 1280px and in the mobile table. Between `md` (768px) and roughly 1016px the SVG scales below 1:1 like any viewBox and its labels render under 14px — about 11.4px at 768px. Inherent to a scaling drawing, and the same trade `DeliveryModelPair` already documents; worth knowing before anyone reads it as a bug.
  - **The axes are `--color-hairline`, not `--color-border`.** The build prompt named border "or the nearest existing token", and border was measured at **1.14:1 against the page in light mode** — the cross was all but invisible at the one breakpoint where it is the entire structure. Hairline measures **1.35:1** and is what `NODE_STYLE` already uses for every drawn stroke in the other diagrams, so this was both the legibility fix and the more consistent choice. Edwin's call after seeing the numbers.
  - **The mobile column headers are "Session initiated by" and "Content set by".** "Session" and "Content" alone were ambiguous about *what* the column was saying — the table's whole subject is who initiates — and the intermediate "Session initiation" still didn't say that the cell below names a party. **The wrap needed forcing.** At 327px both headers wrap to two lines, and left alone they break before "by", orphaning it on a line of its own. The qualifier is therefore bound with a non-breaking space (` `, written as an explicit escape rather than a raw byte so it survives a reformat and is visible when reading the file), which leaves the break after the first word: **"Session" / "initiated by"** and **"Content" / "set by"**. Verified by measuring the rendered line boxes character by character, not assumed. `align-top` holds the two headers on a shared first line instead of one centring against the other, and the mode names carry `whitespace-nowrap` so the wrap only ever happens in those two deliberate places. Above `md` there is room for all three on one line and none of it applies. Screen readers treat the NBSP as a space, so the announced header is unaffected.
  - **Visual 6 has a real caption** — "Cowork isn't a midpoint between the two failures. It's the combination neither of them had." — Edwin's copy, curly apostrophes, and **the first caption on the page that isn't a bracketed TODO.** It states the argument the diagram makes rather than describing what is drawn, which is the job a caption on an argument diagram should do. The other four slots still carry `[ Caption TBC — … ]`.
  - **Verification.** Build and lint clean. Zero `<path>` elements and zero hardcoded hex in the file. In the DOM at 1280: three `<circle>`s only, the vacant corner correctly dotless; axes at `--color-hairline`, 2px; labels resolving to muted, foreground and primary as specified; no horizontal overflow; the mobile table `display: none`. Every `<text>` bounding box sits inside the viewBox with clearance — the left column's labels stop 33 units short of the vertical axis and the pole labels 14 units clear of each axis end. At 375 the table measures 327×196 with three uniform 46px rows, every cell on one line, no cell overflowing its box. **Both themes verified live:** dark returned `#4FA3C0` accent, `#96928A` muted, `#F4F2ED` foreground, `#3F3E38` axes; light returned `#124E66`, `#6B7075`, `#111214`, `#D1D5DB`. Every element resolved through the theme and none kept a colour from the other.
  - **Captions elsewhere are still unwritten.** Visuals 2, 4 and 5 keep their `[ Caption TBC — … ]` placeholders, and the hero keeps its bracketed label. Visuals 3 and 6 have real caption copy, because they are the two that have real visuals. *(Superseded by Prompt 14.3 — 2, 4 and 5 now have both.)*
- [ ] **Prompt 14.3 (Salli visuals 2, 4 and 5 — art-directed screenshots)** — 🚧 **Five of six visuals built. The hero is the last placeholder, so the page is still not deployable.** Six images added under `public/work/salli/`, three `Figure` slots filled, `CaseStudyLayout` extended, and this log updated. Four files plus six images.
  - **`Figure` gained `mobileImage`, and this is the first change to `CaseStudyLayout` since Prompt 10.** The raster variant rendered exactly one `<Image>` — no `<picture>`, no srcset, no breakpoint toggle, and no light/dark concept at any point, which answers a standing question: the leadership study's screenshot passes a single `image` and there has never been a dark alternative to supply. The three Salli screenshots are **art-directed** — a genuinely different crop below `md`, not the same picture scaled — so one source could not serve both. Edwin's call was to put the capability in `Figure` rather than route around it with a `variant="svg"` wrapper: *"the out-of-scope note was to prevent new block types and container widths, not to block a needed capability from living in the right place."* **Strictly additive** — a figure with no `mobileImage` takes the original branch untouched, verified on the leadership study, which still renders zero `<picture>` elements and the identical `className`.
  - **The mechanism is Next's own art-direction pattern**, `getImageProps()` plus `<picture>` with `<source media="(min-width: 768px)">`, from `node_modules/next/dist/docs/01-app/03-api-reference/02-components/image.md` §"Art direction". Both sources still go through the image optimiser; the browser fetches only the one that matches, rather than the `hidden md:block` approach the diagrams use, which ships both. It resolves to a **single `<img>` and therefore a single accessible name**, so `mobileImage` deliberately has no `alt` of its own. `getImageProps` works inside a Client Component — confirmed at build and at runtime, not assumed.
  - **`aspect` turned out to be load-bearing after all, and the build prompt was right.** Three times running, the finding had been that `aspect` is dead once a visual lands. For an art-directed raster it is not: the `<img>` can carry only one pair of width/height attributes — the mobile file's — so above `md` the browser reserved a **portrait** box and would have reflowed when the landscape file arrived. Measured before the fix at a 1280px viewport: boxes at ratio 1.333 and 0.750 where they should have been 1.778 and 1.600. `<source>` also accepts width/height and the spec says it should govern, but it did not hold here and is not dependable enough to hang layout stability on. So the CSS ratio does the work, exactly as the prompt's aspect table specified — `aspect-[4/3] md:aspect-[16/9]` for visual 2, `aspect-[3/4] md:aspect-[16/10]` for 4 and 5 — with `object-cover` as a safeguard; the ratios sit within a tenth of a percent of the files' own, so nothing visible is cropped. The `aspect` doc comment on `FigureBase` was corrected, since it claimed the prop was ignored once a real image landed.
  - **Alt text describes the screenshots, which were read first.** Each names what is actually on screen — the focus list and its ten entries, the alert highlights popover, Cowork's to-do agenda and the £-at-risk conflict card. **Visual 4's two crops show different things** (a web popover versus a push notification over a sales dashboard), and `<picture>` allows only one alt, so that one names both surfaces rather than describing whichever crop the author happened to be looking at.
  - **Verification.** Desktop at 1280 serves `*-web.webp` at 824×464 (16:9) and 824×515 (16:10); mobile at 375 serves `*-mobile.webp` at 327×245 (4:3) and 327×436 (3:4). **No layout shift at either breakpoint** — box dimensions captured before load, with `complete: false`, are byte-identical to those after the images decode. No horizontal overflow. In dark mode both diagrams still resolve through the theme while the screenshots are untouched, with no filter or opacity applied — expected, they are theme-independent product UI. Build and lint clean; the one lint catch was `jsx-a11y/alt-text`, which cannot see `alt` arriving through a spread, so it is now passed explicitly as well.
  - **One expectation in the prompt does not hold:** `grep -c "Caption TBC"` returns **0**, not 1. Only visuals 2, 4 and 5 carried that string, and all three now have real captions. The hero has no caption at all — `heroVisual` takes a `Placeholder` and `CaseStudyLayout` renders no `<figcaption>` for it — so what remains is its *label*, `[ Hero — Salli in-platform ]`. A hero caption was not invented to make the count come out at 1.
- [x] **Prompt 14.4 (Salli hero — the last placeholder)** — ✅ **All six visuals built. No placeholder text remains anywhere on the page**, and what stands between Salli and `main` is now Edwin's review rather than missing work. Two images added under `public/work/salli/`, the `heroVisual` swapped, one word changed in `CaseStudyLayout`, and this log updated. Three files plus two images.
  - **`ArtDirected` is now exported, and that is the whole `CaseStudyLayout` change.** `heroVisual` is typed `ReactNode` and rendered inside a plain `<Container>`, so structurally it accepted a raster figure with no change at all — but the art-direction markup was a module-private function, and the hero could not reach it. The alternatives were both worse: duplicating roughly thirty lines into the content file, including the subtle `<source>` width/height reasoning, or wrapping the hero in a `Figure`, which would have added a `<figcaption>` the hero must not have and constrained it to the reading column. **Direct precedent:** `Placeholder` was already exported from the same file for exactly this reason, and the hero was its only consumer. One word, no logic change, surfaced for a decision rather than worked around.
  - **The hero keeps its `aria-hidden="true"` wrapper, so the alt is not announced today.** That wrapper has been there since Prompt 10, when heroes were decorative diagrams. This hero is real product UI, which is a fair argument for exposing it — but removing the attribute would also expose the leadership study's decorative `ConvergenceHero`, and that is a separate decision. Edwin's call: leave it. The alt is written properly regardless, so it is correct the moment that wrapper changes; it is simply doing no work at present, and this entry says so rather than implying otherwise.
  - **Dimensions confirmed with `sips` before wiring, as instructed.** `salli-hero-web.webp` 2368 × 1015 = 2.3330 against 21:9's 2.3333, and `salli-hero-mobile.webp` 984 × 554 = 1.7762 against 16:9's 1.7778 — 0.01% and 0.09% out, comfortably inside what `object-cover` absorbs. Both match the spec; nothing to report as wrong. **There was no conversion step**, contrary to the prompt's request for before/after sizes: both files arrived as `.webp`, at 67,850 and 23,582 bytes.
  - **`sizes="100vw"`, not the figures' `920px`.** The hero sits at full `Container` width — 1184px of content at a 1280px viewport — rather than the `Measured` column every body figure uses, so it needs its own sizes hint. `aspect="aspect-[16/9] md:aspect-[21/9]"` is carried over unchanged from the placeholder it replaced, so the reserved box is identical to what the page held before.
  - **Verification.** `currentSrc` resolves to `salli-hero-web.webp` at 1280 and `salli-hero-mobile.webp` at 375 — the resolved source, not the markup. **No layout shift at either breakpoint:** the box measures 1184×507 at ratio 2.333 on desktop and 327×184 at 1.778 on mobile, captured while `complete` was `false` and `naturalWidth` `0`, and both deltas are exactly 0 after decode. `grep -iE "TBC|Placeholder"` on the content file returns nothing and the `Placeholder` import is gone. The five body figures and both diagrams measure unchanged, and **the leadership study is untouched** — its hero still renders `ConvergenceHero`, and the page contains zero `<picture>` elements, confirming its raster still takes the original single-`<Image>` branch. Build and lint clean.
  - **A straight apostrophe was caught by the standing grep**, in a code comment rather than in copy — the comment was reworded rather than the character swapped, since the file has held at zero straight apostrophes since it was written and the check is worth keeping absolute.
- [ ] **Prompt 14.2 (Salli visual 3 — the agent grouping)** — 🚧 **Two of six visuals built; the hero and visuals 2, 4 and 5 are still placeholders and the page is still not deployable.** `src/components/diagrams/AgentGrouping.tsx` created, visual 3's `Figure` swapped from a placeholder to the component, and this log updated. Three files.
  - **The argument is that nothing is hidden.** Two panels side by side: on the left every sub-agent exposed as an undifferentiated field, on the right the same marks gathered into ten named focuses. **The mark counts are identical — 120 each side — and only the grouping differs**, because the disagreement was never about how much the system could do, it was about how much of its structure a manager has to hold in their head. The ten runs are `[10, 12, 8, 14, 10, 16, 12, 8, 12, 18]`, deliberately uneven so the groups read as groups rather than as a second, tidier grid. **Raised from 60 to 120 after the first build**, with the runs doubled exactly so the relative sizes are untouched — a denser field makes the left panel read as stock rather than as a countable set, which is what it is for. `TOTAL` is derived from the run lengths rather than declared, so the two panels cannot drift apart.
  - **No arrow, connector, funnel, constriction or stack**, and no accent colour anywhere. The first list is because every one of those devices says one side *becomes* the other, when these are alternatives. The second is the rule that separates this component from `InitiationMatrix`: teal there marks the resolution of a question, but this diagram presents a decision rather than a finding, and tinting either panel would argue for it. Muted, foreground and hairline only — asserted in the DOM in both themes, zero elements resolving to `--color-primary`.
  - **The clusters are single runs, not clumps.** A run reads as a unary bar, so ten unequal groups can be compared at a glance — which is the difference between the panels made visible: one field is uncounted, the other is counted. The labels start at a common x so the unequal runs read against a shared left edge.
  - **The left field is a plain uniform grid — the scatter was built, then removed.** The first version displaced each mark by an index-derived offset so the field wouldn't read as a grid. Edwin's call to take it out, and it is the better one: **a scatter is a pattern too**, and a loose one invites the eye to look for meaning in the gaps, which is the opposite of what this panel is for. Regular and tight reads as undifferentiated stock — nothing in it to attend to. Desktop is **12 × 10 at a uniform 31 step**, spanning **341 of the panel's 356 — 96% of its width** — and closing at y 369 against the tenth row opposite at 387, so the two panels open on the same line and finish within 18 units of each other. Mobile is **20 × 6 at a uniform 15**. Both are square grids, so neither rows nor columns read as the organising direction, and both were asserted regular in the DOM: exactly one distinct gap on each axis, equal on both.
  - **The longest run is what constrains the whole drawing.** At 18 marks, the desktop run step came down from 11 to 8 and the labels moved out to `rightX + 160`; at 11 an 18-mark run would have driven straight through the label column. Mobile is tighter still — 18 marks beside a 14px label in 327 units — so its step is 5.5 and its radius drops to 1.75. Measured clearances: desktop's tightest run-to-label gap is 16 units with the widest label ending at 761 of 800; mobile's is 8.5 with the widest ending at 277 of 327.
  - **The `aspect` prop was dropped, not changed.** The build prompt asked for visual 3's aspect to go from `aspect-[4/3] md:aspect-[16/9]` to `aspect-[3/4] md:aspect-[16/9]` to get a taller mobile box. **`aspect` does nothing once a child is present** — `Figure` only forwards it to `Placeholder`, and that branch is unreachable, so the change would have been a dead prop implying a control that is not wired up. Both `aspect` and `placeholder` come off, exactly as visual 6's did. The taller mobile box comes from the portrait viewBox, which is what actually governs height. Edwin's call after the mechanism was flagged; wiring `aspect` up in `CaseStudyLayout` was rejected as out of scope and as a change to the four remaining placeholder slots.
  - **Mobile is a second SVG at `viewBox="0 0 327 436"`,** not an HTML table as in visual 6 — the content here is a drawing rather than a matrix of values, so it stays a drawing. 327 units wide is chosen because the slot measures 327px at a 375px viewport, so it renders **1:1 and 14 units is a true 14px**. Wrapped in `md:hidden mx-auto max-w-[400px]`, the `TierDecisionFlow` pattern: uncapped it keeps scaling to 719px at the `md` boundary and the type balloons past 30px for no gain. Nothing is truncated — all 60 marks and all ten labels are present at both breakpoints. Ten rows plus two headers in 436 units is the binding constraint; `rowH` came down from 23 to 22 and the second header moved to 176 after the first pass left the last label clearing the bottom edge by 4 units. It now clears by 21.
...
- [x] **Prompt 16 (R&R row out, nav "Work" in)** — ✅ The last two links to `portfolio.edwincw.com` retired together. Three files modified; no images touched.
  - **The row came out rather than being relinked, because there is nowhere yet to relink it to.** Rewards & Recognition is the one case study still on the slide-based portfolio; the leadership study and Salli have both migrated to `/work/[slug]`. Repointing the row at an unmigrated study would have meant either keeping the external hop or linking a page that does not exist, so the row is parked until the study lands. `public/work/rewards-recognition.webp` and `rewards-recognition-dark.webp` are **deliberately left on disk** for that migration — both still present, untouched, and now unreferenced by any component.
  - **`reverse` needed a manual repair for the second time.** The field is written per row and derived from nothing, so any insertion or removal in the middle of the list breaks the alternation. R&R sat at position three carrying `reverse`, so deleting it left `L R L L` — seedbank and FluxUX both image-left, back to back. Fixed by adding `reverse` to FluxUX. **Before: `L R L R L` (leadership, Salli, seedbank, R&R, FluxUX). After naive removal: `L R L L`. Repaired: `L R L R`.** The `reverse` values themselves went `false, true, false, true, false` → `false, true, false, true`. The comment above the list previously recorded only the Prompt 12 leadership insertion as the reason the field had been hand-flipped; it now records both events and says explicitly that `revealIndex` moves with it, so the next edit does not have to rediscover the coupling.
  - **`revealIndex` renumbered `0,1,2,3,4` → `0,1,2,3`.** Only FluxUX changed, `4 → 3`. Contiguity matters because `globals.css` computes `transition-delay: calc(var(--reveal-index, 0) * 80ms)` — a gap would have left an 80ms hole in the cascade. Measured in the rendered DOM: delays resolve to `0s, 0.08s, 0.16s, 0.24s` in document order.
  - **The numbered eyebrow left with the row.** R&R carried `"Case study 02"`, the last numbered eyebrow on the page and the one Prompt 15 deliberately declined to renumber because this removal was already scheduled. The four survivors are all descriptive — "Leadership case study", "Product case study", "Design System Project", "Product innovation project" — and **none was changed**. Source casing varies — "Design System Project" is title case where the others are sentence case — but this is **not a defect and needs no backlog entry**: `.text-eyebrow` sets `text-transform: uppercase` (`typography.css`), so every eyebrow renders uppercase regardless of how it is written in the source. The sentence-case-vs-title-case distinction Prompt 15 drew between row and page eyebrows is a source convention only and has no rendered consequence. Edwin's correction to a first draft of this entry, which had flagged it as an inconsistency worth fixing.
  - **The nav item is `/#selected-work`, rooted rather than bare.** A bare `#selected-work` resolves to nothing from `/about` or `/work/[slug]`, and `Nav` renders on every route. `target="_blank"`, `rel="noopener noreferrer"` and the `ArrowUpRight` were all removed — and the **`ArrowUpRight` import with them**, since Case Studies was its only consumer in this file and an orphaned import fails lint. Confirmed in the rendered DOM: all three nav anchors are same-tab with no `target`, no `rel`, and zero `<svg>` descendants. Work carries no `aria-current` — like Contact it is an in-page anchor, so About remains the only nav item that can ever be active.
  - **`scrollToContact` generalised to `scrollToAnchor(id)`.** Adding Work made Contact no longer "the only in-page destination in the nav", which is what the old block comment claimed. Rather than leave two in-page items behaving differently, the handler became a curried `(id) => (event) => …` used by both. Behaviour is unchanged and deliberately so: it only intercepts when `pathname === "/"` (from any other route the `Link` navigates and the router lands on the anchor), it asks `matchMedia("(prefers-reduced-motion: reduce)")` because a `scrollIntoView` call is not covered by the global reduced-motion reset in `globals.css`, and it uses `history.replaceState` so an in-page move does not cost a back-button press.
  - **`scroll-mt-[80px]`, and the 80 is measured rather than guessed.** `id="selected-work"` **already existed** on the `<Section>` — nothing was added for the anchor target itself, only the offset. The sticky header measured **61.05px** in the browser, which confirms the ~61px recorded in Prompt 9.1's entry (`py-4` + `text-sm` content + 1px border); 80px leaves a **18.95px** buffer. The value is taken from `Contact.tsx`'s existing `scroll-mt-[80px]` rather than derived independently — one number for both anchors, so the two in-page destinations land identically. Note the heading was never actually at risk: `Section` also applies `py-16 md:py-24`, so the eyebrow sits 64–96px below the section element's own top regardless. 80px is for consistency with its sibling, not because the heading needed rescuing.
  - **Verification — the anchor was tested from all three origins, and measured rather than eyeballed.** From `/about` and from `/work/salli-agentic-ai`, clicking Work navigates to `/#selected-work` and lands at `scrollY 1836` with the section's top edge at exactly **80.2px** and the "Selected work" eyebrow at 176.2px against a 61px header — 115px of clearance, heading fully visible. At 375 the same load lands with the section top at **80.1px** and the eyebrow at 144.1px. `scrollIntoView({behavior:'auto'})` on the section resolves to `1916 − 80 = 1836`, confirming `scroll-margin-top` is applied to the real scroll target. Four rows render, R&R absent, image side `LEFT / RIGHT / LEFT / RIGHT` read off `md:order-2`, `--reveal-index` `0,1,2,3`. seedbank and FluxUX keep `target="_blank"`, `rel="noopener noreferrer"` and one `<svg>` arrow each; the two internal rows have none. No horizontal overflow and no nav overflow at 375 or 1280. Console clean. Build and lint clean.
  - **One thing could not be verified in this browser, and it is pre-existing.** Programmatic smooth scrolling is inert in the in-app preview browser — `scrollIntoView({behavior:'smooth'})` sampled every 150ms for 1.8s never moves `scrollY` off 0, while `behavior:'auto'` on the same element jumps correctly. This affects the **existing Contact link identically** (tested directly: same handler, same result), so it is an environment limitation and not a regression from this change. The final resting position is the same either way and was verified through the `auto` path and through real cross-page navigation. The smooth animation itself should be eyeballed once on the Netlify preview.
  - **`portfolio.edwincw.com` in `src/` is now zero — in `docs/` it survives only where it should.** `grep -rn "portfolio\.edwincw\.com" src` returns nothing, and the string is absent from the rendered homepage DOM. In this document it remains in **§1's assets bullet**, where it is the portfolio's actual address and belongs; in **§4 (lines ~103, ~104, ~126)**, the **original locked content spec**, already stale in other ways — it still describes three rows and Salli as external; in **§10's Prompt 6, 9.1, 9.6 and 15 entries**, which **record what was true at the time**; and in this entry, which has to name the thing it retired. That is this document's stated convention, the same one Prompt 15's entry invoked when it left §9 rows 14–14.3 reading "🚧 … still placeholders". Rewriting them would falsify the record. Only §1's assets bullet was updated, because it is maintained current-state and was explicitly forward-looking ("moves once enough case studies have migrated"). Edwin's call, made before implementation.
  - **No git command was run.** `git status` shows exactly three modified files — `src/components/SelectedWork.tsx`, `src/components/Nav.tsx`, `docs/design-brief.md` — nothing staged, nothing else changed.

- [x] **Prompt 17 (Cowork → Focus, focuses → agents)** — ✅ Two linked product renames carried into the Salli case study. Five files modified; no images touched.
  - **The second rename is a consequence of the first, not a separate tidy-up.** Salli's Cowork mode is now Focus, which left "focus" naming both the agent library's items and the mode built to answer their failure — the same word for the thing that failed and the thing that fixes it, which is the exact distinction the essay rests on. So the library's items became plain **agents**, and the sentence that defined them was reworded rather than substituted: "The agents were focuses it could call on, and a manager could switch focus inside a single conversation" → "**The agents were topics it could call on, and a manager could switch agent** inside a single conversation". "Topics" carries the sense "focuses" was doing without reusing the retired term.
  - **One "focus" was deliberately left standing.** The opening section's "what do I need to focus on right now" is the manager's own question in ordinary English, not the product term, and it is the line the whole essay turns on. `src` was swept for other ordinary-English uses before editing: `AboutContent.tsx`'s "especially focused on" and `ConvergenceHero.tsx`'s `focusable="false"` are both outside the case study and both untouched. Nothing else was ambiguous.
  - **`grep -rn "Cowork" src` cannot return nothing, and should not.** The build prompt asked for that check. `RepositioningProductDesignContent.tsx` line 261 reads "It runs in **Claude Cowork** with the Figma MCP" — Anthropic's product, in the leadership case study, which the prompt put out of scope. Renaming it would have been wrong, so the check became `grep -rn "Cowork" src --exclude="RepositioningProductDesignContent.tsx"`, which does return nothing. Flagged before implementation rather than silently satisfied.
  - **Alt text is written to the new name ahead of the plates.** The screenshots still render "Cowork" and are being re-exported separately, so for now the alt describes something the image does not literally say. That is the right trade: the text is correct the moment the new plates land, and a screen reader user is better served by the current product name than by a retired one. Four alts rewritten — the hero, visual 2, visual 5 and the homepage row. Visual 4 (alerts) carried neither term and was not touched. **`ArtDirected` takes `alt` only on `image`, never on `mobileImage`** — one `<picture>` resolves to one `<img>` — so a single string covers both crops in each pair, as Prompt 14.3's entry already recorded.
  - **Visual 2's alt says "agent library", not "agent list".** The first draft used "list", which is what the old string said ("Salli's focus list"). Edwin's correction: the caption on the same figure says "The agent library and in-chat agent switching", the essay uses "library" throughout, and visual 3 is built around the term. One word for one thing across caption, prose and alt.
  - **Neither diagram has an `aria-label`,** which the build prompt asked to update. Both pass `title` and `desc` constants into `DiagramSvg`, which renders SVG `<title>`/`<desc>` — the same intent by a different mechanism, so `TITLE` and `DESC` were updated in both. `InitiationMatrix`'s `DESC` named Cowork twice, `AgentGrouping`'s `TITLE` and `DESC` said "focuses"; the mobile table's `sr-only` `<caption>` named neither and was left alone.
  - **`FOCUSES` became `AGENTS`.** The const escapes a lowercase grep by being uppercase, so nothing forced the rename — but leaving it would have kept the retired concept as the name of the thing in code. The **ten row labels inside it are product UI strings and are byte-identical**, `Labor snapshot`'s US spelling included.
  - **The panel-header contrast improved rather than merely surviving.** "every sub-agent exposed" against "**ten agents exposed**" reads as sub-agent versus agent — one scale of the same noun — where the old pairing set sub-agent against a different word entirely. The left header was not touched. Both headers still end on "exposed", which is what Prompt 14.2's entry established.
  - **Nothing in this document was rewritten.** All nine remaining `cowork`/`focuses` hits in `docs/` were classified by section before deciding: L658 (Prompt 14), L665, L666, L675 (14.1), L693, L701 (14.2), L682 (14.3), L704, L707 (15) — every one inside a §10 Progress Log entry recording what was true at the time. **Zero hits in §1, §2, §3, §4, §8 or §11–14**, the maintained forward-looking sections, so unlike Prompt 16 — where §1's assets bullet did need updating — there was no current-state reference to correct this round. `docs/polish-audit.md` has none. Edwin's call, made before implementation, and the same convention Prompts 15 and 16 both invoked.
  - **Verification.** Build and lint clean. `grep -rn "Cowork" src --exclude="RepositioningProductDesignContent.tsx"` and `grep -rni "focuses" src` both return nothing; `grep -rn "focus" src/app/work/[slug]/SalliAgenticAiContent.tsx` returns the single ordinary-English instance. The two `/work/salli/salli-cowork-*.webp` paths remain — the files are out of scope until re-export, and being lowercase they do not trip the capital-C check. `caseStudies.ts` was checked and needed no change: the Salli meta description carries neither term. Rendered at 1280 and 375, light and dark: both diagrams' new labels in place, no overflow, no clipped text.

- [x] **Prompt 17.1 (Salli "Chat and Focus" — reconciling Chat with the library)** — ✅ Two prose edits in one paragraph. One file modified plus this document; no diagram, caption or alt text touched.
  - **The diagram and the prose disagreed, and the diagram was right.** `InitiationMatrix` plots point 1 as "Agent library", but the paragraph introducing the two modes named Chat as though it were new — so a careful reader had no way to reconcile a three-point diagram with a two-mode model. The fix says the identity out loud rather than redrawing anything: **"The agent library became Chat, unchanged but no longer carrying the whole burden of getting a manager started."** One sentence, inserted after "two modes of the same companion" and before Chat is first characterised. "Unchanged" is load-bearing — the essay's argument is that capability was never the constraint, so the library must not read as having been fixed.
  - **"alongside chat and alerts" → "alongside the library and the alerts".** At that point in the chronology the mode did not exist, so there was nothing called Chat for the new idea to sit alongside — the sentence was describing the moment of the decision in vocabulary that only existed after it. It also removes a lowercase "chat" sitting one sentence from capital-C Chat as a product name, which Prompt 17's rename made newly conspicuous.
  - **Recorded because the paragraph was specified by a build prompt.** Prompt 14 wrote this prose, so anyone re-running a fidelity diff against that prompt will see this block differ, and that is expected. Same reasoning as Prompt 14's own "Three copy corrections after the diff" bullet, which exists for exactly this purpose. Nothing structural moved: one added sentence inside the existing `<Prose>` block, no new block, no `"use client"`, no new imports.
  - **Verification.** `grep -n "alongside chat"` returns nothing. Rendered at 1280 and 375, light and dark: the paragraph grows **6 → 8 lines at 1280** (584px measure, 17px) and **10 → 13 at 375** (327px, 16px) — measured by swapping the old string back in and reading the height, not estimated — and nothing else moved — heading order, figure count and the five reveal targets in this section all unchanged, no horizontal overflow, console clean. Build and lint clean.
