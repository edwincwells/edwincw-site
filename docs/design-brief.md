...
- [x] **Prompt 9.5 (OG image + OpenGraph/Twitter metadata)** — OpenGraph and Twitter Card metadata wired into `src/app/layout.tsx`, referencing `public/og-image.png` (designed and exported separately by Edwin in Figma — 1200×630px PNG, paper-warm background `#F7F5F1`, name in serif italic teal, "Experience Strategy & Product Leadership" tagline in display font, restrained editorial composition matching the site's register). Metadata added: `metadataBase: new URL("https://www.edwincw.com")`, plus `openGraph` block (title, description, url, siteName, images array with width/height/alt, locale `en_GB`, type `website`) and `twitter` block (card `summary_large_image`, title, description, images). Title and description duplicated across `openGraph` and `twitter` — different platforms read different keys and both should match. Image referenced as `/og-image.png` (root-relative URL path, served from `public/` by Next.js). About page metadata in `src/app/about/page.tsx` left unchanged — inherits OG and Twitter from root layout via Next.js metadata merging (child overrides what it specifies, inherits what it doesn't), so sharing `/about` produces the same image as sharing `/`. Per-route OG image overrides explicitly out of scope; single shared image is correct for a two-page site where the OG message is positioning. **Production URL note:** at time of ship, `https://www.edwincw.com` was still serving an older Framer-built site pending DNS cutover to the Next.js/Netlify build. Metadata pre-positioned for production reality rather than retrofitted later — accepts a brief broken-preview window for any Netlify-subdomain links shared in the transitional period (likely none). **Post-cutover action required:** run the LinkedIn Post Inspector on `https://www.edwincw.com` and `https://www.edwincw.com/about` to force LinkedIn to refresh its OG cache (LinkedIn caches aggressively, often 7+ days; Slack/iMessage caches expire on their own). Verified locally: page source on `/` includes all expected `og:*` and `twitter:*` meta tags; OG image loads at `http://localhost:3000/og-image.png`; `npm run build` clean with no `metadataBase` warnings. Live preview testing (LinkedIn Post Inspector, Twitter Card Validator, Facebook Sharing Debugger, manual iMessage/Slack share) gated behind DNS cutover — these tools fetch publicly accessible URLs only. Commits: one local commit.
